...
class IntToBytesMethod(ToBytesMethod):
    ...
    def generation_order(self) -> list[int]:
        ...
        return [big_endian_index, signed_index, length_index, self_index]
    ...
    def generate_internal_opcodes(self, code_generator):
        ...
        # stack: big_endian, signed, length, self

        code_generator.duplicate_stack_top_item()
        # if self == 0: self_bytes = b'\x00' * length
        ...
        # stack: big_endian, self_bytes, length, signed
        ...
        code_generator.convert_literal(self.exception_message + ', negative integers are not supported when unsigned')
        code_generator.convert_raise_exception()
        code_generator.convert_end_if(if_value_is_bigger_than_length, is_internal=True)

        #   last_bytes_self = self_bytes[-1]
        code_generator.duplicate_stack_item(2)
        code_generator.duplicate_stack_top_item()
        ...
        #   if last_bytes_self == 0x00: self_bytes = self_bytes[:-1]
        ...
        #       if first_bytes_self & 0b10000000 != 0 and len(self_bytes) == length: raise Exception
        if_positive_value_is_too_big = code_generator.convert_begin_if()
        code_generator.convert_literal(self.exception_message + ', int too big to convert')
        code_generator.convert_raise_exception()
        ...
        code_generator.swap_reverse_stack_items(3, rotate=True)
        code_generator.swap_reverse_stack_items(3, rotate=True)
        # stack: big_endian, padding, self_bytes, length
