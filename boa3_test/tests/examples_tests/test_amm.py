...
class TestTemplate(BoaTest):
    ...
    def test_amm_onNEP17Payment(self):
        ...
        path = self.get_contract_path('amm.py')
        path_wrapped_tokens = self.get_contract_path('examples/test_native', 'example_contract_for_wrapped_tokens.py')
        path_zneo = self.get_contract_path('wrapped_neo.py')
        path_zgas = self.get_contract_path('wrapped_gas.py')
        engine = TestEngine()
        ...
        output, manifest = self.compile_and_save(path_wrapped_tokens)
        ...
        with self.assertRaises(TestExecutionException, msg=self.ABORTED_CONTRACT_MSG):
            self.run_smart_contract(engine, path, 'onNEP17Payment', test_address, transferred_amount, None,
                                    signer_accounts=[test_address])
        ...
        result = self.run_smart_contract(engine, path_wrapped_tokens, 'calling_transfer',
                                         zneo_address, self.OWNER_SCRIPT_HASH, amm_address, transferred_amount, None,
                                         signer_accounts=[self.OWNER_SCRIPT_HASH],
                                         expected_result_type=bool)
        self.assertEqual(True, result)
    ...
    def test_amm_add_liquidity(self):
        ...
        result = self.run_smart_contract(engine, path_zneo, 'transfer', self.OWNER_SCRIPT_HASH, test_address,
        ...
        result = self.run_smart_contract(engine, path_zgas, 'transfer', self.OWNER_SCRIPT_HASH, test_address,
                                         10_000_000 * 10 ** 8, None,
                                         signer_accounts=[self.OWNER_SCRIPT_HASH],
                                         expected_result_type=bool)
        self.assertEqual(True, result)
        ...
        result = self.run_smart_contract(engine, path_wrapped_tokens, 'calling_approve',
        ...
        balance_user_amm_before = self.run_smart_contract(engine, path, 'balanceOf', test_address)
        reserves_before = self.run_smart_contract(engine, path, 'get_reserves')
        balance_user_zneo_before = self.run_smart_contract(engine, path_zneo, 'balanceOf', test_address)
        balance_user_zgas_before = self.run_smart_contract(engine, path_zgas, 'balanceOf', test_address)
        ...
        result = self.run_smart_contract(engine, path, 'add_liquidity',
                                         transferred_amount_zneo, transferred_amount_zgas, 0, 0, test_address,
                                         signer_accounts=[test_address])
        import math
        ...
        transfer_events = engine.get_events('Mint')
        self.assertEqual(1, len(transfer_events))
        self.assertEqual(3, len(transfer_events[0].arguments))

        address, amount_zneo, amount_zgas = transfer_events[0].arguments
        ...
        balance_user_amm_after = self.run_smart_contract(engine, path, 'balanceOf', test_address)
        reserves_after = self.run_smart_contract(engine, path, 'get_reserves')
        balance_user_zneo_after = self.run_smart_contract(engine, path_zneo, 'balanceOf', test_address)
        balance_user_zgas_after = self.run_smart_contract(engine, path_zgas, 'balanceOf', test_address)
        ...
        # saving data to demonstrate that the value will change later
        total_supply_before = self.run_smart_contract(engine, path, 'totalSupply')
        balance_user_before = self.run_smart_contract(engine, path, 'balanceOf', test_address)
        reserves_before = self.run_smart_contract(engine, path, 'get_reserves')
        balance_user_zneo_before = self.run_smart_contract(engine, path_zneo, 'balanceOf', test_address)
        balance_user_zgas_before = self.run_smart_contract(engine, path_zgas, 'balanceOf', test_address)
        balance_amm_zneo_before = self.run_smart_contract(engine, path_zneo, 'balanceOf', amm_address)
        ...
        # data that will be compared with the previously saved data
        total_supply_after = self.run_smart_contract(engine, path, 'totalSupply')
        balance_user_after = self.run_smart_contract(engine, path, 'balanceOf', test_address)
        reserves_after = self.run_smart_contract(engine, path, 'get_reserves')
        balance_user_zneo_after = self.run_smart_contract(engine, path_zneo, 'balanceOf', test_address)
        balance_user_zgas_after = self.run_smart_contract(engine, path_zgas, 'balanceOf', test_address)
        balance_amm_zneo_after = self.run_smart_contract(engine, path_zneo, 'balanceOf', amm_address)
    ...
    def test_amm_remove_liquidity(self):
        ...
        with self.assertRaises(TestExecutionException, msg=self.ABORTED_CONTRACT_MSG):
            self.run_smart_contract(engine, path, 'remove_liquidity', 10000, 0, 0, test_address,
                                    signer_accounts=[self.OWNER_SCRIPT_HASH])
        ...
        result = self.run_smart_contract(engine, path, 'remove_liquidity', liquidity, 0, 0, test_address,
                                         signer_accounts=[test_address])
        ...
        transfer_events = engine.get_events('Burn')
        self.assertEqual(1, len(transfer_events))
        self.assertEqual(3, len(transfer_events[0].arguments))
    ...
    def test_amm_swap_zneo_to_zgas(self):
        ...
        with self.assertRaises(TestExecutionException, msg=self.ABORTED_CONTRACT_MSG):
            self.run_smart_contract(engine, path, 'swap_tokens',
                                    swapped_zneo, 0, zneo_address, test_address,
                                    signer_accounts=[test_address])
        ...
        result = self.run_smart_contract(engine, path, 'swap_tokens',
                                         swapped_zneo, 0, zneo_address, test_address,
                                         signer_accounts=[test_address], rollback_on_fault=False)
    ...
    def test_amm_swap_zgas_to_zneo(self):
        ...
        with self.assertRaises(TestExecutionException, msg=self.ABORTED_CONTRACT_MSG):
            self.run_smart_contract(engine, path, 'swap_tokens',
                                    swapped_zgas, 0, zgas_address, test_address,
                                    signer_accounts=[test_address])
