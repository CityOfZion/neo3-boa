...
class TestRuntime(boatestcase.BoaTestCase):
    from boa3.internal.model.builtin.interop.interop import Interop

    default_folder: str = 'test_sc/runtime_test'
    ...
    async def test_contract_with_check_witness(self):
        await self.set_up_contract('CheckWitness.py')
        call_contract = await self.compile_and_deploy('test_sc/utils_test', 'CallScriptHash.py')
    ...
    async def test_import_sc_runtime(self):
        await self.set_up_contract('ImportScRuntime.py')
    ...
    async def test_get_signers(self):
        ...
        self.assertEqual('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00',
                         signer[account_hash])
    ...
    async def test_get_signers_compare_signer(self):
        ...
        result, _ = await self.call('compare_scope', [verification.WitnessScope.GLOBAL], return_type=bool,
                                    signers=[signer])
        self.assertEqual(True, result)
