...
class TestStorage(boatestcase.BoaTestCase):
    from boa3.internal.model.builtin.interop.interop import Interop
    default_folder: str = 'test_sc/storage_test'
    ...
    async def test_storage_find_with_options(self):
        ...
        for (key, value) in expected_result:
            await self.call('put_on_storage', [(prefix + key), value], return_type=None,
    ...
    async def test_boa2_storage_test(self):
        ...
        result, _ = await self.call('main', ['sput', storage_key, 'blah'], return_type=bool,
                                    signing_accounts=[self.genesis])
        self.assertEqual(True, result)
        ...
        result, _ = await self.call('main', ['sdel', storage_key, 'blah'], return_type=bool,
                                    signing_accounts=[self.genesis])
        self.assertEqual(True, result)
    ...
    async def test_boa2_storage_test2(self):
        ...
        result, _ = await self.call('main', ['sput', storage_key, value], return_type=bool,
                                    signing_accounts=[self.genesis])
        self.assertEqual(True, result)
        ...
        result, _ = await self.call('main', ['sdel', storage_key, value], return_type=bool,
                                    signing_accounts=[self.genesis])
        self.assertEqual(True, result)
    ...
    async def test_create_map(self):
        ...
        result, _ = await self.call('insert_to_map', [storage_key, stored_value], return_type=None,
                                    signing_accounts=[self.genesis])
        self.assertIsNone(result)
    ...
    async def test_import_interop_storage(self):
        await self.set_up_contract('ImportScStorage.py')
    ...
    async def test_as_read_only(self):
        ...
        result, _ = await self.call('put_value_in_storage', [key, value_old], return_type=None,
                                    signing_accounts=[self.genesis])
        self.assertIsNone(result)
        ...
        with self.assertRaises(boatestcase.FaultException) as context:
            await self.call('put_value_in_storage_read_only', [key, value_new], return_type=None,
                            signing_accounts=[self.genesis])
